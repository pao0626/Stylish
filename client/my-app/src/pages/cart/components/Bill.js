...
import { API_POST_Pay } from "../../../global/constants";
...
//SetupSDK
  ...
  function checkout() {
    ...
    if (!window.TPDirect.card.getTappayFieldsStatus().canGetPrime) {
      window.alert('付款資料輸入有誤');
      return;
    }

    async function checkoutResult(data) {
      const res = await fetch(`${API_POST_Pay}`, {
          body: JSON.stringify({
                prime: data.card.prime,
                order: {
                  shipping: 'delivery',
                  payment: 'credit_card',
                  subtotal,
      ...
      <div className="cart__items">
        {props.cartItems.map((item, index) => (
          <div className="cart__item" key={index}> 
            <img src={item.image} className="cart__item-image" alt="購物車商品照片"/>
            <div className="cart__item-detail">
              <div className="cart__item-name">{item.name}</div>
              <div className="cart__item-id">貨號：{item.id}</div>
              <div className="cart__item-color">顏色｜
                <div className="cart__color-block"
